...
class TestPosts:
    ...
    def test_get_all_posts(self):
        ...
        logger.info(f"Get All Posts Response Code: {response.status_code}")
        posts = response.json()
        assert len(posts) > 0, f"Get All Posts Length Greater Than 0: {len(posts) > 0}"
        logger.info(f"Get All Posts Length Greater Than 0: {len(posts) > 0}")
        first_post = posts[0]
    ...
    def test_get_posts_by_id_valid(self):
        ...
        logger.info(f"Get Posts By ID Response Code: {response.status_code}")
        post = response.json()
        ...
        logger.info(f"ID Requested: {id}, Post ID returned is: {post["id"]}")
    ...
    def test_create_post_valid(self):
        ...
        logger.info(f"Create Post Response code: {response.status_code}")
        post = response.json()
        assert post["userId"] == user_id, f'User ID Expected: {user_id}, User ID Returned: {post["userId"]}'
        logger.info(f'User ID Expected: {user_id}, User ID Returned: {post["userId"]}')
        assert post["title"] == title, f'Title Expected: {title}, Title Returned: {post["title"]}'
        logger.info(f'Title Expected: {title}, Title Returned: {post["title"]}')
        assert post["body"] == body, f'Body Expected: {body}, Body Returned: {post["body"]}'
        logger.info(f'Body Expected: {body}, Body Returned: {post["body"]}')
    ...
    def test_partial_update_post(self):
        ...
        logger.info(f"Partial Update Post Status Code: {response.status_code}")
        post = response.json()
        assert post["userId"] == post_user_id, f'User ID Expected: {post_user_id}, User ID Returned: {post["userId"]}'
        logger.info(f'User ID Expected: {post_user_id}, User ID Returned: {post["userId"]}')
        assert post["title"] == post_title, f'Title Expected: {post_title}, Title Returned: {post["title"]}'
    ...
    def test_delete_post_valid_id(self):
        ...
        logger.info(f"Delete Post Status Code: {response.status_code}")
    ...
    def test_get_post_comments_valid(self):
        ...
        logger.info(f"Get Post Comments Status Code: {response.status_code}")
        comments = response.json()
        assert len(comments) > 0, f"Get All comments Length Greater Than 0: {len(comments) > 0}"
        logger.info(f"Get All comments Length Greater Than 0: {len(comments) > 0}")
        ...
        logger.info(f"First Comment Post ID: {first_comment["postId"]}, Expected Post ID: {post_id}")
